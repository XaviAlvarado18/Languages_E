...
def evaluate_text(text):
    ...
    clear()
    evaluated_text = exclusiveSim(machine, text)[:-1]
    new_eval = []
    pass_rules = False

    for message, token in evaluated_text:
        if token != 1 and token != 0:
            if (token == 'TOKENS' or token == 'FIRST') and not pass_rules:
                new_eval.append((message, 1))
                continue
            elif token == 'RULES':
                pass_rules = True
            elif token == 'DECLARATIONS' and pass_rules:
                pass_rules = False
                is_first = False
        ...
        if token == 'HEADER':
            if not find_header:
                ...
                if '\{' in message or '\}' in message:
                    ...
                    sep_to_escape_pre = message.split('\{')
                    sep_to_escape = []
                    for sep in sep_to_escape_pre:
                        sep_to_escape += sep.split('\}')

                    for sep in sep_to_escape:
